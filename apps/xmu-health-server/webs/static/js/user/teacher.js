...
function to_page(pageNum) {
    ...
    $.ajax({
        url: common_ops.buildUrl("/user/getTeacherList"),
        data:data,
        type: "POST",
        dataType: "json",
    ...
    function build_table(data) {
        ...
        if(teacher.length==0){
            ...
            $("<tr></tr>").append(item).appendTo("#teacher_table tbody");
            return;
        }
        ...
        $.each(teacher, function (index, item) {
            ...
            $("<tr></tr>").append(seq).append(dept).append(no).append(name).append(op).appendTo("#teacher_table tbody");
...
//用户详情Modal
$("#teacher_table").on("click",".detail",function(){
    ...
    //清空table表格
    $("#teacher_track_table tbody").empty();
...
function to_tracklist_page(no,pageNum) {
    ...
    $.ajax({
        url: common_ops.buildUrl("/track/getTeacherTrackByNo"),
        data:data,
        type: "POST",
        dataType: "json",
        success: function (data) {
    ...
    function build_table(data) {
        ...
        if(tracklist.length==0){
            ...
            $("<tr></tr>").append(item).appendTo("#teacher_track_table tbody");
            return;
        }
        var offset = (data.pages.current-1)*data.pages.page_size
        //遍历元素
        $.each(tracklist, function (index, item) {
            ...
            $("<tr></tr>").append(seq).append(place).append(type).append(time).appendTo("#teacher_track_table tbody");
        })
    }
