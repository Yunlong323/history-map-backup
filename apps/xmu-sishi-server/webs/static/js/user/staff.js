...
function to_page(pageNum) {
    ...
    $.ajax({
        url: common_ops.buildUrl("/user/getStaffList"),
        data:data,
        type: "POST",
        dataType: "json",
    ...
    function build_table(data) {
        ...
        if(staff.length==0){
            ...
            $("<tr></tr>").append(item).appendTo("#staff_table tbody");
            return;
        }
        ...
        $.each(staff, function (index, item) {
            ...
            $("<tr></tr>").append(seq).append(dept).append(no).append(name).append(op).appendTo("#staff_table tbody");
...
//用户详情Modal
$("#staff_table").on("click",".detail",function(){
...
function to_tracklist_page(no,pageNum) {
    ...
    $.ajax({
        url: common_ops.buildUrl("/track/getStaffTrackByNo"),
        data:data,
        type: "POST",
        dataType: "json",
        success: function (data) {
    ...
    function build_table(data) {
        ...
        if(tracklist.length==0){
            ...
            $("<tr></tr>").append(item).appendTo("#staff_track_table tbody");
            return;
        }
        var offset = (data.pages.current-1)*data.pages.page_size
        //遍历元素
        $.each(tracklist, function (index, item) {
            ...
            $("<tr></tr>").append(seq).append(place).append(type).append(time).appendTo("#staff_track_table tbody");
        })
    }
